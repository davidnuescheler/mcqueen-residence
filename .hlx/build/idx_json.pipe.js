...
const {
  Pipeline
} = require('@adobe/helix-pipeline/index');

const {
  log
} = require('@adobe/helix-pipeline/src/defaults/default');
...
const {
  selectstrain,
  selecttest
} = require('@adobe/helix-pipeline/src/utils/conditional-sections');
...
const {
  cache,
  uncached
} = require('@adobe/helix-pipeline/src/html/shared-cache');
...
const jsonpipe = (cont, context, action) => {
  ...
  const timer = timing();
  pipe.every(dump.record).every(validate).when(() => !production()).every(timer.update).use(resolveRef).expose('resolve').when(hascontent).use(fetch).expose('fetch').when(hascontent).use(parse).expose('parse').use(parseFrontmatter).use(embeds).use(smartypants).use(sections).use(meta).expose('meta').use(unwrapSoleImages).use(selectstrain).use(selecttest).use(html).expose('html').use(sanitize).when(paranoid).use(cont).use(type('application/json')).use(cache).when(uncached).use(key).use(debug).use(timer.report).error(dump.report).error(selectStatus);
  action.logger.log('debug', 'Running custom JSON pipeline');
  return pipe.run(context);
};

module.exports.pipe = jsonpipe;
//# sourceMappingURL=idx_json.pipe.js.map