...
function pre(context) {
  ...
  const $sections = $(document.body).children('div'); // first section has a starting image: add title class and wrap all subsequent items inside a div

  $sections.first().has('p:first-child>img').addClass('title').find(':nth-child(1n+2)').wrapAll('<div class="header"></div>'); // sections consisting of only one image

  $sections.filter('[data-hlx-types~="has-only-image"]').not('.title').addClass('image'); // sections without image and title class gets a default class

  $sections.not('.image').not('.title').addClass('default'); // if there are no sections wrap everything in a default div
...
module.exports.before = {
  fetch: async (context, action) => {
    ...
    logger.info(`resourcePath=${resourcePath}`); // sanitize the mountpoints
    ...
    secrets.REPO_RAW_ROOT = secrets.GOOGLE_DOCS_ROOT || 'https://adobeioruntime.net/api/v1/web/helix/helix-services/gdocs2md@latest'; // ump the timeout a bit, since the google docs script might take a while to render
...
//# sourceMappingURL=html.pre.js.map