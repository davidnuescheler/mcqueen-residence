...
async function fetch(context, action, resourcePath, rootId) {
  ...
  const requestId = request.headers['x-request-id'] || request.headers['x-cdn-request-id'] || request.headers['x-openwhisk-activation-id'] || '';
  ...
  try {
    ...
    if (e.statusCode === 404) {
      ...
      logger.error(`Error while fetching Markdown from ${options.uri} with the following ` + `options:\n${inspect(options, {
        depth: null
      })}`);
...
//# sourceMappingURL=fetch-google.js.map